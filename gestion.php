...
<form method="post" action="acceuil_connect.php" enctype="multipart/form-data">
    ...
    <h3> Livres </h3>
    <a href = "modifications_livre.php"> Modifier les données </a> </br>
    <!-- Trié par auteur -->
    <?php
    try
    {
    ...
    $pre = "";
        while ($row = $reponse->fetch(PDO::FETCH_ASSOC))
        {
           $pre .= "<option value='".$row['prenom']."'>".$row['prenom']."</option>";
        }?>

        <label for "categorie"> Par prénom :</label> <br />
        <select name = "prenom[]" id = "categorie">
            <option value="Tous">Tous</option>
            <?php echo $pre; ?>
        </select> <br />
    ...
    $nom = "";
        while ($row = $reponse->fetch(PDO::FETCH_ASSOC))
        {
           $nom .= "<option value='".$row['nom']."'>".$row['nom']."</option>";
        }?>

        <label for "categorie"> Par nom :</label> <br />
        <select name = "nom[]" id = "categorie">
            <option value="Tous">Tous</option>
            <?php echo $nom; ?>
        </select> <br />
    ...
    $mai = "";
        while ($row = $reponse->fetch(PDO::FETCH_ASSOC))
        {
           $mai .= "<option value='".$row['mail']."'>".$row['mail']."</option>";
        }?>

        <label for "categorie"> Par nom :</label> <br />
        <select name = "mail[]" id = "categorie">
            <option value="Tous">Tous</option>
            <?php echo $mai; ?>
        </select> <br />
    ...
    $mess = "";
        while ($row = $reponse->fetch(PDO::FETCH_ASSOC))
        {
           $mai .= "<option value='".$row['message']."'>".$row['message']."</option>";
        }?>

        <label for "categorie"> Par nom :</label> <br />
        <select name = "message[]" id = "categorie">
            <option value="Tous">Tous</option>
            <?php echo $mai; ?>
        </select> <br />

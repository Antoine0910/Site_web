...
?>
    ...
    Catégorie : <?php echo $donnees['categorie']; ?>
        ...
        {
            $id_Livre = $id_Livre_requet['id_Livre'];


        while ($message_rep = $message_pers->fetch())
        { 
            if(isset($donnees['id_Livre']) == $id_Livre AND $message_rep['id_Livre'] == $id_Livre)
            {
            ?>
        <p>

            >> Message : <?php echo $message_rep['message']; ?> </br>
            Posté par <?php echo $message_rep['prenom'] .' '. $message_rep['nom']; ?> 
            le <?php echo $message_rep['date_message']; ?> </br>

        </p>

        <?php }
        else {
            echo "Pas de réponses pour ce sujet";
        } ?>
        ...
        <form method="post" action="test18.php" enctype="multipart/form-data">
            <p>
